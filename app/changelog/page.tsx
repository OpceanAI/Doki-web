    ...
    <main className="min-h-screen bg-[var(--bg-000)]">
      ...
      <div className="max-w-[var(--max-width)] mx-auto px-4 sm:px-6 pt-32 pb-16">
        <div className="text-center">
          ...
          <div className="inline-flex items-center gap-2 badge-enhanced mb-6">
            <span className="status-dot" />
            <span className="text-[var(--text-300)] font-mono">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-32">
        {releases.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-[var(--text-500)]">No releases found.</p>
            <Link href="https://github.com/OpceanAI/Doki/releases" target="_blank" className="text-[var(--accent-cyan)] hover:underline text-sm mt-2 inline-block">
              View on GitHub
            </Link>
          </div>
        ) : (
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-[var(--border-100)]" />
              ...
              return (
                <div key={release.id} className={`relative flex items-start gap-6 mb-12 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                  ...
                  <div className={`absolute left-4 md:left-1/2 -translate-x-1/2 w-3 h-3 rounded-full border-2 z-10 ${
                    isLatest ? "bg-[var(--accent-cyan)] border-[var(--accent-cyan)]" : "bg-[var(--bg-000)] border-[var(--border-200)]"
                  }`} />

                  {/* Content */}
                  <div className={`ml-12 md:ml-0 md:w-[calc(50%-2rem)] ${i % 2 === 0 ? "md:pr-8 md:text-right" : "md:pl-8"}`}>
                    <div className="p-6 bg-[var(--bg-100)] border border-[var(--border-100)] rounded-xl hover:border-[var(--border-200)] transition-colors">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <span className={`font-mono text-sm font-medium ${isLatest ? "text-[var(--accent-cyan)]" : "text-[var(--text-200)]"}`}>
                          {release.tag_name}
                        </span>
                        {isLatest && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--accent-cyan)]/20 text-[var(--accent-cyan)] font-medium">
                            LATEST
                          </span>
                        )}
                        {release.prerelease && (
                      ...
                      <h3 className="font-semibold text-[var(--text-100)] mb-1 text-sm">
                        {release.name}
                      </h3>

                      <p className="text-xs text-[var(--text-500)] mb-3">
                      ...
                      {highlights.length > 0 && (
                        <ul className={`space-y-1 mb-4 ${i % 2 === 0 ? "md:text-right" : ""}`}>
                          {highlights.map((h, j) => (
                            <li key={j} className="text-xs text-[var(--text-400)] leading-relaxed">
                              {h}
                            </li>
                          ))}
                        </ul>
        ...
        {/* CTA */}
        <div className="text-center pt-8 border-t border-[var(--border-100)]">
          <p className="text-sm text-[var(--text-500)] mb-4">
          ...
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="cta-pill-secondary text-sm"
          >
